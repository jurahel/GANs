...
class Crypto():
	def __init__(self,data_path):
		...
		self.data = df[df.Symbol=='BTCUSD'].Close.reset_index(drop=True).head(500)
	...
	def plot_ts(self):
		...
		plt.imshow(self.data[:rows*cols].reshape((rows,cols)))
		plt.savefig('images/TimeSeriesImage.png')
		plt.close()
...
class WGANGP():
	def __init__(self,seq_len):
		self.seq_len = seq_len
		self.img_rows = self.seq_len
		self.img_cols = 1#int(np.sqrt(self.seq_len))
		self.channels = 1
		...
		#fake_img = K.expand_dims(fake_img, axis = 2)
	...
	def build_critic(self):
		print('Critic')
		model = Sequential()
		model.add(Dense(200, input_shape=(144,1)))
		model.add(Activation('relu'))
