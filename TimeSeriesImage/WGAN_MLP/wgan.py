...
class Crypto():
    ...
    def reshape_as_img(self):
        ...
        for seq in self.data_seq:
            ...
            imgs.append(seq.reshape((self.col,self.row)))
...
class WGAN():
    def __init__(self,row,col):
        ...
        optimizer = RMSprop(lr=0.00005)# standard: lr=0.00005
    ...
    def train(self, epochs, batch_size, sample_interval):
        ...
        for epoch in range(epochs):
            ...
            for _ in range(self.n_critic):
                ...
                idx = np.random.randint(0, btc.imgs.shape[0], batch_size)
                imgs = X_train[idx]
            ...
            if (epoch%sample_interval==0)&(epoch>0):
                ...
                ax[0].imshow(gen_imgs[0].reshape(btc.col,btc.row))
...
if __name__ == '__main__':
    #prepare data
    df=pd.read_csv('ETH_BTC.csv',sep=';')
    btc=Crypto('Bitcoin',df[df.Symbol=='BTCUSD'].Close.reset_index(drop=True).head(5000))
    seq_len = 784
    #btc.data=btc.ret
    btc.scale(-1,1)
    ...
    plt.show()
    '''
    wgan.train(epochs=1, batch_size=20, sample_interval=10)
